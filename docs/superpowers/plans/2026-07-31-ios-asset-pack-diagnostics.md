...
- [x] Write Swift tests for invalid manifest, missing asset, size mismatch, all-valid assets, and stable JSON keys using fixture directories.
- [x] Run `swift test --package-path packages/assets_util/ios/assets_util` and verify the new tests fail because the diagnostics builder does not exist.
- [x] Add a platform-neutral manifest/file inspector and JSON encoder in `EQMAssetsUtil.swift`.
- [x] Add the iOS-only `AssetPackManager` diagnostics, including the iOS 26.4 availability signal without early-returning before file inspection.
- [x] Add the completion-handler bridge for `checkForUpdates()`; return sorted IDs and NSError domain/code/description in JSON.
- [x] Run `swift test --package-path packages/assets_util/ios/assets_util` and verify all tests pass.
...
- [x] Write Dart tests with literal schema-v1 JSON for every top-level status, per-file state, nullable paths, native errors, unknown schema, and update success/failure.
- [x] Run `mise exec -- dart test packages/assets_util/test/asset_pack_diagnostics_test.dart` and verify failure because models are missing.
- [x] Implement strict JSON decoding with descriptive `FormatException`; preserve all available diagnostic fields.
- [x] Generate the Objective-C header and bindings through `packages/assets_util/hook/build.dart` or `tool/generate_ios_bindings.dart`; do not hand-edit generated selectors or blocks.
- [x] Bridge the synchronous diagnostics NSString and asynchronous completion block to typed Dart results with `Completer`.
- [x] Expose the methods only for iOS; other platforms throw `UnsupportedError` without changing `resolvePackRoot()`.
- [x] Run package tests and `mise exec -- dart analyze packages/assets_util`.
...
- [x] Write repository tests proving partial diagnostics are returned when the pack is not ready and manifest metadata is parsed when present.
- [x] Write Action tests proving only explicit execution calls update, success invalidates diagnostics, and failure preserves the prior diagnostics state.
- [x] Run the two tests and verify they fail because repository/action types are missing.
- [x] Implement repository/provider separation and the Mutation-backed Action.
- [x] Run `mise exec -- dart run build_runner build --delete-conflicting-outputs` from `app/`.
- [x] Run repository/action tests and verify they pass.
...
- [x] Write widget tests for unsupported OS, manifest missing, native error, partial file list, size mismatch, ready state, update progress/result/error, refresh-only behavior, and textScale 2.0 overflow.
- [x] Run the widget test and verify failure against the old page.
- [x] Replace the single `_NotReady` branch with a diagnostics summary that always renders available evidence.
- [x] Add copyable OS/pack/path/error tiles and the explicit 「更新を確認」 button; disable it during Mutation pending.
- [x] Preserve the right-top refresh action as provider invalidation only.
- [x] Run the widget test and verify it passes.
...
- [x] Run Swift tests.
- [x] Run assets_util tests and analyze.
...
whole-file rewrite. Full `app/` analysis was attempted twice but produced no result after
approximately 7 minutes each; targeted analysis of all changed Asset Pack app files passed.
