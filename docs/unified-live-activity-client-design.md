...
### 2026-09-21 デザインレビュー反映

- EEW が主表示の場合は、Lock Screen・Dynamic Island の全領域で既存 EEW View を再利用する。統合側独自の予想 Chip は追加しない。
- 揺れ検知の本文は検知時刻と現在地のみ。headline と重複する揺れの強さ、更新時刻、検知終了の説明文は表示しない。ピーク値の保持は継続する。
- 地震情報の取消表示は設けない。デザイン確認用一覧から地震情報取消・ブロック欠損のサンプルを削除する。
- Lock Screen は OS の高さ制限（160pt）を考慮し、見出しに含まれる震源名を本文で繰り返さず、バッジと余白を縮める。
- Expanded は種別・見出しと現在地の1行に絞る。最大震度と規模／深さは leading / trailing に配置し、発生時刻・EEW 副次帯・大きな現在地バッジは本文に追加しない。

以下の初期案と競合する場合は、このレビュー結果を優先する。

### 追加レビュー

- `VXSE51` が `informationType` に含まれる場合、headline 右の最大震度は省く。
- headline の文字サイズは16pt、上下余白は8pt、外周余白は12pt。縞は5ptとする。
- 地震情報の震源要素は EEW と同じ View で表示する。Mと深さは一体で縮小し、値だけの省略と文字の重なりを防ぐ。M8以上は `M8+` とする。
- EEW の副次帯には予想最大震度に加え、Mと深さを表示する。種別名は `EEW` と略す。
- EEW 主表示の到達カウントダウンは18pt、現在地の予想震度は48ptに縮める。
- 揺れ検知から EEW への切り替えでは、黒背景の親 View を維持する。
...
- 種別ラベルは `informationType` 配列から最も情報量の多いものを選ぶ。優先度 `VXSE53 > IXAC41 > VXSE52 > VXSE51`。
  `VXSE51` = 震度速報 / `VXSE52` = 震源に関する情報 / `VXSE53` = 震源・震度に関する情報 / `IXAC41` = 推計震度分布。
- magnitude は `NORMAL` → `M6.8`、`UNKNOWN` → `M不明`、`OVER_M8` → `M8以上`、`null` → 行ごと非表示。
  `OVER_M8` は巨大地震なので、M 表示自体を強調色にする。
- 現在地の `earthquake.location.maxIntensity` は**観測**震度。バッジ左上に `観測` Chip を重ね、EEW の予想震度と取り違えないようにする。配色は `IntensityValue` の共通配色を使う。
