...
@objc public final class CustomStyleLayer: MLNCustomStyleLayer {
    private var pipelineState: MTLRenderPipelineState?
    private var depthStencilStateWithoutStencil: MTLDepthStencilState?

    @objc override public init(identifier: String) {
        super.init(identifier: identifier)
    }

    @objc public static func initWithIdentifier(_ identifier: String) -> CustomStyleLayer {
        return CustomStyleLayer(identifier: identifier)
    }
    ...
    public override func draw(in mapView: MLNMapView, with context: MLNStyleLayerDrawingContext) {
        #if MLN_RENDER_BACKEND_METAL
            guard let renderEncoder = self.renderEncoder,
                  let pipelineState = self.pipelineState,
                  let depthStencilStateWithoutStencil = self.depthStencilStateWithoutStencil else {
                return
            }
    ...
    public func project(_ coordinate: CLLocationCoordinate2D) -> CGPoint {
    ...
    public func convertMatrix(_ mat: MLNMatrix4) -> MLNMatrix4f {
        MLNMatrix4f(
