...
struct StoredDeviceLocationSyncLease {
    let leaseId: String
    let updateId: String
}

final class DeviceLocationSyncLeaseStore {
    private static let lock = NSLock()
    private static let leaseIdKey = "blt_device_location_sync_lease_id"
    private static let updateIdKey = "blt_device_location_sync_lease_update_id"
    private static let expiresAtMillisKey = "blt_device_location_sync_lease_expires_at_millis"

    private let userDefaults: UserDefaults
    private let nowMillis: () -> Int64
    private let leaseIdProvider: () -> String
    ...
    init(
        ...
        leaseIdProvider: @escaping () -> String = { UUID().uuidString }
    ) {
        self.userDefaults = userDefaults
        self.nowMillis = nowMillis
        self.leaseIdProvider = leaseIdProvider
    }

    func acquire(updateId: String, durationMillis: Int64) -> StoredDeviceLocationSyncLease? {
        Self.lock.lock()
        defer { Self.lock.unlock() }
        let now = nowMillis()
        let expiresAt = userDefaults.object(
            forKey: Self.expiresAtMillisKey
        ) as? Int64 ?? 0
        guard expiresAt <= now else {
            return nil
        }
        let lease = StoredDeviceLocationSyncLease(
            leaseId: leaseIdProvider(),
            updateId: updateId
        )
        userDefaults.set(lease.leaseId, forKey: Self.leaseIdKey)
        userDefaults.set(lease.updateId, forKey: Self.updateIdKey)
        userDefaults.set(now + durationMillis, forKey: Self.expiresAtMillisKey)
        return userDefaults.synchronize() ? lease : nil
    }

    func isOwned(leaseId: String, updateId: String) -> Bool {
        Self.lock.lock()
        defer { Self.lock.unlock() }
        let expiresAt = userDefaults.object(
            forKey: Self.expiresAtMillisKey
        ) as? Int64 ?? 0
        return userDefaults.string(forKey: Self.leaseIdKey) == leaseId
            && userDefaults.string(forKey: Self.updateIdKey) == updateId
            && expiresAt > nowMillis()
    }

    func release(leaseId: String) -> Bool {
        Self.lock.lock()
        defer { Self.lock.unlock() }
        guard userDefaults.string(forKey: Self.leaseIdKey) == leaseId else {
            return false
        }
        userDefaults.removeObject(forKey: Self.leaseIdKey)
        userDefaults.removeObject(forKey: Self.updateIdKey)
        userDefaults.removeObject(forKey: Self.expiresAtMillisKey)
