...
void main() {
  group('restoreResponse', () {
    ...
    test('unknown responseType defaults to JSON parsing', () {
      final options = RequestOptions(path: '/test');
      final entry = HttpCacheEntry(
        key: 'test-key',
        statusCode: 200,
        ...
        body: Uint8List.fromList(utf8.encode('{"fallback":true}')),
        updatedAtMs: 0,
      );
      ...
      expect(response.statusCode, 200);
      expect(response.data, {'fallback': true});
    });

    test('empty JSON object restores correctly', () {
      final options = RequestOptions(path: '/test');
      final entry = HttpCacheEntry(
        key: 'test-key',
        statusCode: 200,
    ...
    test('JSON array restores correctly', () {
      final options = RequestOptions(path: '/test');
      final entry = HttpCacheEntry(
        key: 'test-key',
        statusCode: 200,
        ...
        headers: {},
        responseType: 'json',
        body: Uint8List.fromList(utf8.encode('[1,2,3]')),
        updatedAtMs: 0,
      );
    ...
    test('unicode content in plain text restores correctly', () {
      final options = RequestOptions(path: '/test');
      final entry = HttpCacheEntry(
        key: 'test-key',
        statusCode: 200,
        ...
        headers: {},
        responseType: 'plain',
        body: Uint8List.fromList(utf8.encode('日本語テスト 🌍')),
        updatedAtMs: 0,
      );

      final response = restoreResponse(options, entry);

      expect(response.data, '日本語テスト 🌍');
    });

    test('unicode content in JSON restores correctly', () {
      final options = RequestOptions(path: '/test');
      final entry = HttpCacheEntry(
        key: 'test-key',
        statusCode: 200,
        ...
        headers: {},
        responseType: 'json',
        body: Uint8List.fromList(utf8.encode('{"msg":"地震速報"}')),
        updatedAtMs: 0,
      );

      final response = restoreResponse(options, entry);

      expect(response.data, {'msg': '地震速報'});
    });

    test('multi-value headers are preserved', () {
      final options = RequestOptions(path: '/test');
      final entry = HttpCacheEntry(
        key: 'test-key',
        statusCode: 200,
