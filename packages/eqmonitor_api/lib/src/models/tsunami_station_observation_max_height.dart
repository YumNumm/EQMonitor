...
@Freezed()
abstract class TsunamiStationObservationMaxHeight
    with _$TsunamiStationObservationMaxHeight {
  const factory TsunamiStationObservationMaxHeight({
    ...
    /// 数値情報に付加的情報が必要な場合に出現
    @JsonKey(includeIfNull: false, name: 'is_rising') dynamic isRising,
    @JsonKey(includeIfNull: false) ObservationMaxHeightCondition? condition,

    /// 欠測によりデータが現在取得できていない場合に出現する
    @JsonKey(includeIfNull: false, name: 'is_missing') dynamic isMissing,
    @JsonKey(includeIfNull: false) Revise? revise,
  }) = _TsunamiStationObservationMaxHeight;

  factory TsunamiStationObservationMaxHeight.fromJson(
    Map<String, Object?> json,
  ) => _$TsunamiStationObservationMaxHeightFromJson(json);
}
