...
class FnetCatalogParser {
  ...
  FnetEvent? _parseEvent(String line) {
    ...
    final strike = FnetAnglePair.fromString(parts[6]);
    final dip = FnetAnglePair.fromString(parts[7]);
    final rake = FnetAnglePair.fromString(parts[8]);

    // 科学的記法の値をパース (例: 1.89e+15)
    final seismicMoment = double.parse(parts[9]);
    final mtDepth = double.parse(parts[10]);
    final momentMagnitude = double.parse(parts[11]);
    ...
    final momentTensor = FnetMomentTensor(
      mxx: mxx,
      mxy: mxy,
      mxz: mxz,
      myy: myy,
