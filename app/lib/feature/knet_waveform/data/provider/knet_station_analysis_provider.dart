...
Future<KnetStationAnalysis> knetStationAnalysis(
  ...
  return compute(_runAnalysis, result);
}

KnetStationAnalysis _runAnalysis(KnetStationResult result) {
  ...
  final accel = List<List<double>>.generate(nCh, (ch) {
    ...
    final bias = ch < record.offsets.length && record.offsets[ch] != 0.0
        ? record.offsets[ch]
        : _meanOfFirst(raw, (raw.length * 0.1).round().clamp(1, raw.length));
...
double _meanOfFirst(List<double> data, int count) {
  if (data.isEmpty) {
    return 0;
  }
  var sum = 0.0;
