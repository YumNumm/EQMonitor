...
void main() async {
  const baseV1 = EarthquakeV1(eventId: 20240102235959, status: '通常');
      ...
      jmaCodeTableProvider.overrideWithValue(
        JmaCodeTable(
          areaEpicenter: AreaEpicenter(
            items: [
              AreaEpicenter_AreaEpicenterItem(code: '100', name: '石狩地方北部'),
            ],
          ),
  ...
  testWidgets("遠地地震の場合、'遠地地震'が表示されること(20210121212726)", (tester) async {
    ...
    await tester.pumpWidget(
      buildWidget(EarthquakeHistoryListTile(item: v1Extended)),
    );
    // Assert
    expect(find.text('遠地\n地震'), findsOneWidget);
  });
  testWidgets("大規模な噴火の場合、'大規模な噴火'が表示されること(20210813033300)", (tester) async {
    // Arrange
    final v1Extended = EarthquakeV1Extended(
      earthquake: baseV1.copyWith(
        text: '''
...
（注２）火山噴火に伴う潮位変化の呼称については、今後検討していきますが、当面は防災対応の呼びかけとして「津波」と表記します。''',
  ...
  group('ListTile.trailing(マグニチュード)', () {
    testWidgets('マグニチュードがある場合、マグニチュードが表示されること', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(magnitude: 5.1),
        maxIntensityRegionNames: null,
      );
      // Act
      await tester.pumpWidget(
      ...
      expect((trailingText! as Text).data, 'M5.1');
    });
    testWidgets('マグニチュードが整数の場合、小数第1位まで表示されること', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(magnitude: 5),
        maxIntensityRegionNames: null,
      );
      // Act
      await tester.pumpWidget(
      ...
      final trailingText = listTile.trailing;
      expect(trailingText, isA<Text>());
      expect((trailingText! as Text).data, 'M5.0');
    });
    testWidgets('マグニチュードが小数第2位以降ある場合、四捨五入されて小数第1位まで表示されること(繰り下げケース)', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(magnitude: 5.123),
        maxIntensityRegionNames: null,
      );
      // Act
      await tester.pumpWidget(
    ...
    testWidgets('マグニチュードが小数第2位以降ある場合、四捨五入されて小数第1位まで表示されること(繰り上げケース)', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(magnitude: 5.162),
        maxIntensityRegionNames: null,
      );
      // Act
      await tester.pumpWidget(
      ...
      final trailingText = listTile.trailing;
      expect(trailingText, isA<Text>());
      expect((trailingText! as Text).data, 'M5.2');
    });
  });
  group('ListTileの背景', () {
    testWidgets('背景色塗りつぶし無効の場合、背景色が塗りつぶされないこと', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(maxIntensity: JmaIntensity.fiveLower),
        maxIntensityRegionNames: null,
      );
      // Act
      await tester.pumpWidget(
    ...
    group('背景色塗りつぶし有効の場合、背景色が透明度40%で塗りつぶされること', () {
      for (final maxIntensity in JmaIntensity.values) {
        ...
        testWidgets('最大震度$maxIntensityの場合', (tester) async {
          // Arrange
          final v1Extended = EarthquakeV1Extended(
            earthquake: baseV1.copyWith(maxIntensity: maxIntensity),
            maxIntensityRegionNames: null,
          );
  ...
  group('ListTile.title部分', () {
    testWidgets('震源地名がある場合、震源地名が表示されること', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(epicenterCode: 100),
    ...
    testWidgets('補助震源地名がある場合、補助震源地名も表示されること', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(
          epicenterCode: 100,
      ...
      expect((titleText! as Text).data, '石狩地方北部(米国、アラスカ州中央部)');
    });
    testWidgets('震源地名が無いが、最大震度とその観測地域(複数)がある場合、最大震度とその観測地域が表示されること', (
      ...
      final titleText = listTile.title;
      expect(titleText, isA<Text>());
      expect((titleText! as Text).data, '最大震度5弱を静岡県伊豆などで観測');
    });
    testWidgets('震源地名が無いが、最大震度とその観測地域(1つ)がある場合、最大震度とその観測地域が表示されること', (
      ...
      final titleText = listTile.title;
      expect(titleText, isA<Text>());
      expect((titleText! as Text).data, '最大震度5弱を静岡県伊豆で観測');
    });
  });
  group('ListTile.subtitle部分', () {
    testWidgets('地震発生時刻がある場合、地震発生時刻が表示されること', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(
      ...
      expect(text.data, startsWith('2024/01/02 23:59頃発生'));
    });
    testWidgets('地震発生時刻がなく、検知時刻がある場合、検知時刻が表示されること', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(
          eventId: 20240102235959,
      ...
      expect(wrap.children[0], isA<Text>());
      final text = wrap.children[0] as Text;
      expect(text.data, startsWith('2024/01/02 23:59頃検知'));
    });
    testWidgets("深さ0kmの時、'深さ ごく浅い'が表示されること", (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(depth: 0),
        maxIntensityRegionNames: [],
      );
      ...
      expect(wrap.children[0], isA<Text>());
      final text = wrap.children[0] as Text;
      expect(text.data, endsWith('深さ ごく浅い'));
    });
    testWidgets('深さ10kmの時、深さ 10kmが表示されること', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(depth: 10),
        maxIntensityRegionNames: [],
      );
      ...
      expect(wrap.children[0], isA<Text>());
      final text = wrap.children[0] as Text;
      expect(text.data, endsWith('深さ 10km'));
    });
    testWidgets('深さ700kmの時、深さ 700km以上が表示されること', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(depth: 700),
        maxIntensityRegionNames: [],
      );
      ...
      expect(wrap.children[0], isA<Text>());
      final text = wrap.children[0] as Text;
      expect(text.data, endsWith('深さ 700km以上'));
    });
    testWidgets('最大長周期地震動階級が0の場合、Chipが表示されないこと', (tester) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
        earthquake: baseV1.copyWith(maxLpgmIntensity: JmaLgIntensity.zero),
        maxIntensityRegionNames: [],
      );
    ...
    group('最大長周期地震動階級が0以外の場合', () {
      for (final intensity in [...JmaLgIntensity.values]
        ...
        testWidgets('最大長周期地震動階級が$intensityの場合、Chipとそのラベルが表示されること', (
          ...
          // Arrange
          final v1Extended = EarthquakeV1Extended(
            earthquake: baseV1.copyWith(maxLpgmIntensity: intensity),
            maxIntensityRegionNames: [],
          );
          ...
          expect(label.data, '最大長周期地震動階級 $intensity');
        });
      }
    });
  });
  testWidgets('タップ時にonTapが呼ばれること', (tester) async {
    // Arrange
    final v1Extended = EarthquakeV1Extended(
      earthquake: baseV1.copyWith(eventId: 20240102235959),
      maxIntensityRegionNames: [],
    );
    var isTapped = false;
    // Act
