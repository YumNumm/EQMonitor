...
void main() async {
  ...
  testWidgets("遠地地震の場合、'遠地地震'が表示されること(20210121212726)", (
    ...
    await tester.pumpWidget(
      buildWidget(
        EarthquakeHistoryListTile(item: v1Extended),
      ),
    );
    // Assert
    expect(find.text('遠地\n地震'), findsOneWidget);
...
（注２）火山噴火に伴う潮位変化の呼称については、今後検討していきますが、当面は防災対応の呼びかけとして「津波」と表記します。''',
  ...
  group('ListTile.trailing(マグニチュード)', () {
    testWidgets('マグニチュードがある場合、マグニチュードが表示されること', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
    ...
    testWidgets('マグニチュードが整数の場合、小数第1位まで表示されること', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
  ...
  group('ListTileの背景', () {
    testWidgets('背景色塗りつぶし無効の場合、背景色が塗りつぶされないこと', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
  ...
  group('ListTile.title部分', () {
    ...
    testWidgets('補助震源地名がある場合、補助震源地名も表示されること', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
    ...
    testWidgets(
      '震源地名が無いが、最大震度とその観測地域(複数)がある場合、最大震度とその観測地域が表示されること',
      (tester) async {
        // Arrange
        final v1Extended = EarthquakeV1Extended(
          earthquake: baseV1.copyWith(
    ...
    testWidgets(
      '震源地名が無いが、最大震度とその観測地域(1つ)がある場合、最大震度とその観測地域が表示されること',
      (tester) async {
        // Arrange
        final v1Extended = EarthquakeV1Extended(
          earthquake: baseV1.copyWith(
  ...
  group('ListTile.subtitle部分', () {
    testWidgets('地震発生時刻がある場合、地震発生時刻が表示されること', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
    ...
    testWidgets('地震発生時刻がなく、検知時刻がある場合、検知時刻が表示されること', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
    ...
    testWidgets("深さ0kmの時、'深さ ごく浅い'が表示されること", (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
    ...
    testWidgets('深さ700kmの時、深さ 700km以上が表示されること', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
    ...
    testWidgets('最大長周期地震動階級が0の場合、Chipが表示されないこと', (
      tester,
    ) async {
      // Arrange
      final v1Extended = EarthquakeV1Extended(
