...
/// テスト用ヘルパー: 最低限のフィールドで [EarthquakeTelegramBodyIntensityRegion] を生成
...
/// テスト用ヘルパー: [EarthquakeTelegramBodyQuake] を生成
...
void main() {
  group('computeIntensityRegionDiff', () {
    test('初報 (previous=null) の場合、全て same になる', () {
      ...
      final result = computeIntensityRegionDiff(
        current: current,
        previous: null,
      );
    ...
    test('震度上方修正 (value4 -> value5minus) を検出する', () {
      ...
      expect(result[0].intensity, api.JmaIntensity.value5minus);
      expect(result[0].previousIntensity, api.JmaIntensity.value4);
    });
    ...
    test('震度下方修正 (value5plus -> value4) を検出する', () {
      ...
      expect(result[0].intensity, api.JmaIntensity.value4);
      expect(result[0].previousIntensity, api.JmaIntensity.value5plus);
    });
  ...
  group('computeHypocenterDiff', () {
    test('差分なしの場合 null を返す', () {
      ...
      final result = computeHypocenterDiff(
        current: quake,
        previous: quake,
      );
    ...
    test('最大震度の変化を検出する', () {
      ...
      expect(result.oldMaxIntensity, api.JmaIntensity.value4);
      expect(result.newMaxIntensity, api.JmaIntensity.value5minus);
    });
