...
void main() {
  ...
  test('realtime refresh is not overwritten by pending pagination', () async {
    final controller = StreamController<RealtimeEvent>.broadcast(sync: true);
    addTearDown(controller.close);
    final adapter = _ControllableTelegramListAdapter();
    final dio = Dio(BaseOptions(baseUrl: 'https://example.com'))
      ..httpClientAdapter = adapter;
    final container = ProviderContainer(
      overrides: [
    ...
    final pagination = container.read(provider.notifier).fetchNextData();
    await adapter.paginationRequested.future;
    controller.add(
      RealtimeEvent.earthquakeUpsert(
        record: _TelegramListFixtures.earthquake(eventId: 'event-1'),
        source: RealtimeSource.eqmonitor,
...
final class _ControllableTelegramListAdapter implements HttpClientAdapter {
  final paginationRequested = Completer<void>();
  final _paginationResponse = Completer<ResponseBody>();
  var _firstPageRequestCount = 0;
  ...
  Future<ResponseBody> fetch(
    ...
    if (options.queryParameters['cursor'] != null) {
      paginationRequested.complete();
      return _paginationResponse.future;
    }

    _firstPageRequestCount += 1;
    final id = _firstPageRequestCount == 1
        ? 'telegram-initial'
        : 'telegram-fresh';
    final nextToken = _firstPageRequestCount == 1 ? 'cursor-1' : null;
    return Future.value(_responseBody(nextToken: nextToken, id: id));
  }

  void completePagination() {
    _paginationResponse.complete(
      _responseBody(nextToken: null, id: 'telegram-stale'),
    );
  }
...
ResponseBody _responseBody({required String? nextToken, required String id}) =>
    ResponseBody.fromString(
      jsonEncode(_responseJson(nextToken: nextToken, id: id)),
      200,
      headers: {
        Headers.contentTypeHeader: [Headers.jsonContentType],
      },
