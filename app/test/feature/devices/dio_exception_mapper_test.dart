...
void main() {
  ...
  for (final type in [
    ...
    test('$type は NetworkUnreachableException', () {
      expect(
        DioExceptionMapper.map(_typed(type)),
  ...
  test('401 は unauthenticated (再試行不可)', () {
    final mapped = DioExceptionMapper.map(_badResponse(401));
    expect(mapped, isA<AuthorizationException>());
    expect(
      (mapped as AuthorizationException).reason,
  ...
  test('403 は forbidden', () {
    final mapped = DioExceptionMapper.map(_badResponse(403));
    expect(mapped, isA<AuthorizationException>());
    expect(
      (mapped as AuthorizationException).reason,
  ...
  test('429 で Retry-After が無ければ retryAfter は null', () {
    final mapped = DioExceptionMapper.map(_badResponse(429));
  ...
  test('400 は InvalidRequestException', () {
    final mapped = DioExceptionMapper.map(_badResponse(400));
  ...
  test('422 は InvalidRequestException', () {
    expect(
      DioExceptionMapper.map(_badResponse(422)),
  ...
  test('未知のステータス (418) は UnexpectedProvisioningException', () {
    expect(
      DioExceptionMapper.map(_badResponse(418)),
  ...
  test('badResponse で statusCode が null なら Unexpected', () {
    ...
    expect(DioExceptionMapper.map(e), isA<UnexpectedProvisioningException>());
  });

  test('unknown タイプは UnexpectedProvisioningException', () {
    expect(
      DioExceptionMapper.map(_typed(DioExceptionType.unknown)),
      isA<UnexpectedProvisioningException>(),
    );
  });
}
