...
@available(iOS 16.1, *)
struct LiveActivityMaximumIntensityView: View {
    let intensity: IntensityValue?
    let size: CGFloat
    var inlineMaximumLabel = false
    var accessibilityTitle = "最大予想震度"
    ...
    var body: some View {
        ...
        .accessibilityLabel("\(accessibilityTitle) \(intensity?.displayString ?? "未発表")")
    }
}

@available(iOS 16.1, *)
struct LiveActivityLocalIntensityView: View {
    let intensity: IntensityValue
    let size: CGFloat
    var containerRelative = false
    var accessibilityTitle = "現在地の予想震度"
    ...
    var body: some View {
        ...
        .background {
            if containerRelative {
                ContainerRelativeShape().fill(backgroundColor ?? intensity.backgroundColor)
            } else {
                RoundedRectangle(cornerRadius: size * 0.2).fill(backgroundColor ?? intensity.backgroundColor)
            }
        }
        .accessibilityElement(children: .ignore)
        .accessibilityLabel("\(accessibilityTitle) \(intensity.displayString)")
    }
}
